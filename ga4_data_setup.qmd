...
execute:
  ...
  message: false
  echo: fenced
  cache: false # ← see discussion below about when to use cache: true vs false
  freeze: auto
---
...
BILLING_PROJECT <- "your-actual-project-id" # ← CHANGE THIS LINE with your real project ID
...
#| label: my-project-id
#| echo: false
BILLING_PROJECT <- "marketing-analytics-491507"
```
------------------------------------------------------------------------
...
::: callout-important
## Important Update: Remove `page_size` from `collect()`

-  `page_size` was previously included in the `collect()` call to control how many rows are fetched per API request. However, with the BigQuery Storage API streaming enabled via `bigrquerystorage`, `collect()` now handles pagination automatically and efficiently without needing a `page_size` argument.
...
tbl(con, "events_20201101") |>
  ...
  collect(page_size = 1000) # Excludes event_params and items — much smaller
...
### `cache: true` vs `cache: false` in Quarto

Caching saves the **results of a code chunk to disk** so it does not re-run on every render. Here is when to use each:
...
| Scenario | Example |
|----------|---------|
| **Slow computations** that rarely change | BigQuery downloads, model fitting |
| **Static data processing** with fixed inputs | Reading and cleaning a fixed dataset |
| **Expensive visualizations** | Complex ggplot with many layers |

```r
#| cache: true
# Good candidate - slow and result never changes
ga4_raw <- bq_table_download(tb, bigint = "numeric")
```
...
| Scenario | Example |
|----------|---------|
| **duckplyr/Arrow objects** | Cannot be serialized to disk |
| **Database connections** | `con` objects are session-specific |
| **Random results** | `sample()`, `set.seed()` |
| **Side effects** | Creating folders, saving files |
| **Fast code** | No benefit from caching |

```r
#| cache: false
# Bad candidate for caching - duckplyr object
ga4_duck <- duckplyr::read_parquet_duckdb("data/ga4_parquet")
```
...
```
Slow + stable result    → cache: true
Fast code               → cache: false  (default)
Database/Arrow objects  → cache: false  (always)
Side effects            → cache: false  (always)
```
...
```r
#| cache: true   # ❌ Never cache these
con     <- dbConnect(duckdb())           # connection = session specific
ga4_duck <- as_duckplyr_df(ga4_raw)     # duckplyr   = cannot serialize
ga4_arrow <- as_arrow_table(ga4_raw)    # arrow      = cannot serialize
```
